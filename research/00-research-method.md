...
已有原型的 RGB、NIR 和距离传感器不应只是“更多 sensor”。多光谱掌纹文献已长期表明可见光与 NIR 看到的线纹/皮下信息不同，因此“融合 RGB 与 NIR 提高识别”本身也不是新颖。可研究的增量应是：在**低成本、固定几何、边缘端、主动短序列、攻击条件**下，何时采集第二模态，怎样拒绝低质量或可疑采集，以及这种 gate 的代价。NIR 对假体/贴片一定有效尚未被本轮文献证明。`E1 + E2 + Q`
...
可撤销模板很重要，但同时要证明跨设备鲁棒、不可逆、不可关联和可撤销，且需要密码学威胁模型，FYP 风险较高。单纯部署优化已有强先例，论文性弱。主动多传感风险门控既复用已有硬件，又能同时量化识别、质量、攻击和边缘延迟；当前是主候选，但只在 demo 后继续。
