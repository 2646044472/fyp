...
| 排名 | 方向 | 为什么值得做 | 主要风险 |
| ---: | --- | --- | --- |
| 1 | **缺失模态/质量感知的多模态 edge vision** | 直接继承 PAMI 的 incomplete multi-view、quality-aware fusion 和 edge 系统问题；可用公开 RGB-D/多视图数据，之后再接实验室 RGB/NIR/ToF 设备。 | 置零一个模态不够新；必须测相关缺失、质量退化、拒答、恢复和能耗。 |
| 2 | **无标签测试时适应 + 安全回滚** | OFTTA 已证明 edge 上无梯度适应可行；新的问题是伪标签污染、最坏时间窗口、reset/rollback，而不是重新命名 TTA。 | 适应可能越适应越错；需要严格 streaming protocol。 |
