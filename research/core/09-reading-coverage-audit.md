...
| 证据账本条目 | 95 | 只收入会改变题目、协议、指标或范围的资料；本轮新增缺失模态综述、CVPR sensor-failure benchmark、CAFuser/MoME、EfficientAD、Real-IAD、BoTTA、OD-TTA、ELaTTA、连续 TTA survey、PAMI 的 TIMVC-IGC/FADNet、sensor-health systematic review、Zhiwei 和单相机 health benchmark 边界。 |
| 已查看关键章节的原始/官方资料 | 约 95 | 在原有掌纹/PAD/edge/标准资料基础上，逐段查看缺失模态综述的 taxonomy/open issues、传感器失败 benchmark 与代码 README、CAFuser 方法/消融、Real-IAD 数据/协议、EfficientAD 方法/效率/限制、CMF/Source-restoration/IST/BoTTA/OD-TTA/ELaTTA 的 TTA 机制和设备约束、TIMVC-IGC/FADNet 的问题边界、sensor-health review、Zhiwei 和单相机 health benchmark，以及掌纹综述的 open-set/data/security outlook。部分只能取得摘要/官方元数据；未把 GPU/医疗私有数据/研究组网页描述当作当前 FYP 实证。 |
