...
| [COLFISPOOF contactless fingerprint PAD](https://openaccess.thecvf.com/content/WACV2023W/MAP-A/papers/Kolberg_COLFISPOOF_A_New_Database_for_Contactless_Fingerprint_Presentation_Attack_Detection_WACVW_2023_paper.pdf) / [official partitions](https://github.com/dasec/COLFISPOOF) | WACV Workshops 2023 | `E1` 论文 + official data/partition artifact | 7,200 PA sample、72 PAI species、两种手机；其 random baseline 让同一 species 进入所有 split，而 LOO 把一个完整 material/appearance group 只留给 test。 | 未知攻击不能只靠随机抽帧定义。它是指纹而非掌纹，不能直接训练/比较，但为本项目的 PAIS/material/capture-chain/session 层级留出提供可执行反例。 |
| [Yao et al., domain-adversarial palmprint anti-spoofing](https://doi.org/10.1109/ICIP49359.2023.10223182) / [IEEE poster record](https://resourcecenter.ieee.org/conferences/icip-2023/spsicip23vid0167) | ICIP 2023 | `E2` metadata/poster abstract + artifact search | 作者称构建 large-scale palmprint attack dataset，并把 domain generalization 与 online triplet mining 引入 PAD；正文受限。以作者、题名和 dataset 反查未发现官方 data page、license、code、weights 或下载入口。 | 它足以否定“首次跨域 PAD”，但 dataset size、PAIS、split、指标和模型结果都不可从摘要严谨复述；不把它误列成公开 benchmark。 |
...
| [HiChrom-MAE](https://doi.org/10.1145/3805622.3810599) | ICMR 2026 | `E2` 正式 metadata/abstract | 论文在七个 domain 上讨论跨介质 palmprint PAD，以 high-frequency residual reconstruction 和 chromaticity-distribution alignment 学习较稳健表征，意图降低光照/背景 artefact 的敏感性。 | 这进一步否定“frequency/chromaticity PAD”是空白；但当前未取得正文、数据或代码，不能复述 PAIS、切分、数值或资源，也不能将其 RGB PAD 结果扩展到 Pi、NIR/ToF、主动挑战或 IAPMR。 |
| [PVASD palm-vein anti-spoofing](https://doi.org/10.1109/TIFS.2025.3650391) / [official code and data card](https://github.com/valhongli/PVASD) | TIFS 2026 | `E1` 全文关键章节 + repository/dataset/requirements audit | 论文引入 5,515 subject、约 118.8 万图、16 种 2D/3D attack、五分辨率的非接触**掌静脉** PAD 数据，并给出 preprocessing、APCER/BPCER/ACER benchmark。当前 repo 有 MIT code、学术/非商业数据规则、三个 Drive split、checkpoint links；安装锁定 CUDA 12.1，训练代码含大 batch 的 GPU path。 | 近期 NIR/静脉 PAD 已有大型、可审计的 data+code 近邻；不再将“缺 NIR PAD benchmark”泛化为创新。它没有 identity matcher/IAPMR、RGB/NIR/ToF synchronization、Pi/ARM profile 或本机 capture service，故只借其 split/PAIS 分层思路，不能直接运行或迁移其数字。 |
| [PalmRSS single-source domain generalization](https://doi.org/10.1016/j.patcog.2025.111620) / [authors' repository](https://github.com/yocii/PalmRSS) | Pattern Recognition 2025 | `E2` 正式摘要 + repo tree/code audit；正文/data/license/weights 未得 | 正式摘要把 device/environment domain shift 作为 palm recognition 的 SSDG 问题，以 source-subset alignment、Fourier/histogram transforms 和 adversarial learning 做 cross-dataset recognition。repo 当前可读但仅有路径 lists/脚本；未见 license、图像数据或权重，默认 CUDA、作者绝对路径和大 batch。 | 跨设备/环境留出已经是正式 research problem，故本项目不能以同设备随机切分的高分代表泛化。它不回答 palmprint PAD、`1:1` IAPMR、RGB/NIR/ToF actual state 或 Pi 部署；只能强化 B0--M 的 session/sensor holdout 纪律。 |
...
| [NIST SP 800-63B-4](https://pages.nist.gov/800-63-4/sp800-63b.html) / [SP 800-116r1](https://csrc.nist.gov/pubs/sp/800/116/r1/final) | current guidance / facility-access scope | `E1` 官方资料 | 将 biometric comparison、physical authenticator、fallback、PAD 和 endpoint integrity 分层；facility access 是 risk-based credential/PACS system。 | 不把 QR/work order + palm 写作 MFA，也不把 capture PAD 写成 endpoint/relay/tailgating defense。原型以 `claim -> local 1:1 -> release/retry/manual fallback` 呈现，终端攻击面单列 residual risk。 |
...
| [Liu et al. edge-computing compact system](https://doi.org/10.1360/SST-2021-0223) | 2022 | `E1` 摘要/出版社片段 | 明确以 edge computing 构建终端、边缘服务器、云三层掌纹识别子系统；终端做 Tiny YOLOv3/MobileNetV2 ROI，边缘端做 GoogLeNet 特征匹配，云端记录与更新。 | 这已经覆盖 edge/offloading 叙事；本次未取得完整 PDF，不能外推具体实时性、硬件、资源、PAD 或安全。若保留 edge 题目，必须定义为本地 capture-to-decision 并用实测和攻击协议区分。 |
...
| [Pi palm-vein prototypes (2017/2019)](https://ijaas.iaescore.com/index.php/IJAAS/article/download/9470/7878) / [NoIR example](https://ijsrd.com/articles/IJSRDV7I40667.pdf) | 2017/2019 | `E1` 全文 | 已将 Raspberry Pi、NIR/NoIR、IR LED、ROI 和 local palm-vein matcher 做成低成本、受控盒式 prototype。 | edge hardware box 已有长线先例。它们缺 modern/reproducible ARM artifact、palmprint `1:1` protocol、PAIS/PAD 和 end-to-end resource profile，不能成为 B0 performance comparison。 |
