...
12. **候选已从系统 demo 改为研究问题，但第二轮审计进一步收紧了它们。** P/R/T/O 都有强近邻，不能以多模态、缺模态、异步、uncertainty、三态或 cross-domain 自称首创。2026-08 的 GBU-Palm 已直接覆盖原生 RGB-NIR 视频、攻击 lineage、环境留出及光谱/时序分析；掌纹只剩“PAD 加冻结 1:1 matcher 的系统级 IAPMR 与端侧成本”这一尚未确认的数据/协议交集。详见 [`16-research-first-direction-set.md`](16-research-first-direction-set.md)、[`39-problem-driven-second-audit.md`](../log/39-problem-driven-second-audit.md) 和 [`40-gbu-palm-deep-read-log.md`](../log/40-gbu-palm-deep-read-log.md)。旧系统优先短名单和故障溯源审计保留为反例，不作为默认选择。
