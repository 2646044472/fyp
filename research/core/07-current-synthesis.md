...
12. **候选已从系统 demo 改为研究问题。** 当前保留一个掌纹题和三个非掌纹题：双轴开放世界 palm PAD/IAPMR、证据约束的跨模态复原、时间不确定性感知 fusion、normality-shift vs defect 三态 anomaly。每项先有可推翻假设和直接近邻审计；只有通过选定方向的完整 novelty gate 才进入原型。详见 [`16-research-first-direction-set.md`](16-research-first-direction-set.md) 与 [`38-research-first-candidates-log.md`](../log/38-research-first-candidates-log.md)。旧系统优先短名单和故障溯源审计保留为反例，不作为默认选择。
