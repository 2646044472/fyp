...
12. **现在不应再停留于泛泛的候选列表。** 当前固定为一个掌纹题和三个非掌纹题：掌纹条件式采集/拒答、传感器故障溯源/剩余可观测性/安全降级、TTA 安全回滚、开放世界工业异常拒答。其中允许换题时优先验证传感器故障溯源方向；若老师要求掌纹，使用同一“失效输入下安全降级”的问题迁移到掌纹设备。每项的最小实验、数据许可、创新边界与停止条件见 [`13-fyp-direction-shortlist.md`](13-fyp-direction-shortlist.md)，该首选的范围限定新颖性审计见 [`15-novelty-audit-sensor-provenance.md`](15-novelty-audit-sensor-provenance.md)。
