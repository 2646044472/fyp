...
12. **候选已从系统 demo 改为研究问题，第三轮审计也改变了非掌纹排序。** P/T/O 都有强近邻，不能以多模态、缺模态、异步、uncertainty、三态或 cross-domain 自称首创；R 的 restoration-hallucination/证据约束又有医学 reconstruction、FDA assessment 与 data-consistency 的强先例，故降为备用。相反，S 不再泛称“失效闭环”，而明确为 `sensor state -> observability -> task risk -> action cost` 的联合事件协议：2026 系统综述也指出这四层常被断开、公开 benchmark 罕有同步标签，而 MAMMOTH 的强近邻只做整路 dropout。2026-08 的 GBU-Palm 已直接覆盖原生 RGB-NIR 视频、攻击 lineage、环境留出及光谱/时序分析；掌纹只剩“PAD 加冻结 1:1 matcher 的系统级 IAPMR 与端侧成本”这一尚未确认的数据/协议交集。详见 [`16-research-first-direction-set.md`](16-research-first-direction-set.md)、[`39-problem-driven-second-audit.md`](../log/39-problem-driven-second-audit.md)、[`40-gbu-palm-deep-read-log.md`](../log/40-gbu-palm-deep-read-log.md) 和 [`42-sensor-state-action-audit.md`](../log/42-sensor-state-action-audit.md)。
