...
合适的第一场景是**机房/设施维修的授权进入或关键工具领用**，而不是支付和城市级监控。它自然需要 `1:1 verification`：使用者先出示工单/二维码/卡片得到匿名 claim，设备再核验“声称身份是否真的到场”。这比 `1:N` 搜索更利于隐私、延迟和 FYP 数据规模控制。它也有比单纯门锁更清晰的价值：离线时仍能把“人、工单、时间、设备/工具”关联成最小审计事件；但这项价值仍须访谈确认。
...
| 临时/受限现场会有身份核验流程 | 常识性业务假设，尚未访谈。`Q` | 等待时间、人工工时、每次进场失败/争议 | 卡片转借、弱网、人工核验究竟发生多少。`Q` |
| 离线模板核验可少传输原始图像 | 可由本系统架构保证。`E1`（设计能力） | 原始图上传次数=0；模板大小；离线成功率 | 这是否产生客户愿付费价值。`Q` |
...
| B. 主动多传感 PAD gate | 单帧 RGB 核验面对重放/贴片时，ToF + RGB/NIR 短序列能否降低误放行 | 采集盒指引距离；正常、打印、屏幕、普通贴片的 through/reject 对照 | 未见攻击材料上的 APCER/BPCER，以及相同 FAR 下攻击成功率与 p95 延迟 | 缺公开数据，要合规自采；NIR 不一定有效 | **主候选** |
...
继续 B 的最低条件：正常用户体验没有明显恶化，并出现可以量化的攻击/质量增益。否则保留 C 为清晰的嵌入式测量项目，或与老师讨论改为 A。

## 7. 下一次访谈：六个问题

1. 哪一种受限区域最常有临时/外包人员？现时怎样确认“人和凭证是同一人”？
