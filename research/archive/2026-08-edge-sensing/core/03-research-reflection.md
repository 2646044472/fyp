...
最后更新：2026-08-27。本文件不是宣传稿，而是用来阻止项目沿着看似合理、其实已被文献或现实否定的方向投入时间。第二轮逐篇阅读的详细记录见 [`../log/35-research-reflection-round2-log.md`](../log/35-research-reflection-round2-log.md)。
...
## 8. 第二轮反思：四个候选其实是一个失效闭环

第二轮阅读后，需要修正“缺失模态、TTA、异常检测是三个新算法方向”的说法。2024--2025 的缺失模态综述、CVPR 传感器失败 benchmark、CAFuser/MoME、EfficientAD、Real-IAD、CMF 和 source-restoration 工作已经分别覆盖了质量感知融合、失效评测、条件式路由、轻量异常、样本级工业评测和 TTA 恢复。单独复现任何一个模块都不足以构成 FYP 创新。

当前更稳健的抽象是：

> observe sensor state -> estimate quality/uncertainty -> choose fuse/degrade/reacquire/adapt/abstain -> log risk and cost。

掌纹保留题目和三个非掌纹候选都应围绕这条闭环定义变量。真正可防守的贡献需要同时满足：现实而非随机的失效机制、最终任务风险或资源改变、未见设备/环境/故障留出，以及 Pi/CPU 上的实测。按此标准，A 的算法新颖性降为中等但启动成本最低；P 的算法新颖性中等、系统验证价值较高但硬件依赖强；T 的创新依赖长期 streaming protocol；O 的创新依赖未知缺陷和端侧拒答，而不是 AUROC 或 SAM 移植。

这不是把方向缩小成“工程没有研究价值”，而是把研究问题从网络名字上移到系统可证伪变量：复杂策略是否真的比固定 fallback 更可靠，是否值得它的采集、延迟、能耗和人工重试成本。详细证据和负结果标准见 [`../log/35-research-reflection-round2-log.md`](../log/35-research-reflection-round2-log.md)。
