...
澳门的正确位置是**约束与访谈地点**，不是未经证明的市场空白：已有澳门银河掌纹支付先例，同时澳门个人资料指引要求考虑用途、比例性、替代方案和留存。密码学 access-control 文献也区分敏感工作区（身份披露可被需要）与公共通行/消费（可关联性不应交给 verifier）。因而 demo 不做支付、全员考勤、执法或劳动资格判断，更不声称 privacy-preserving/non-transferable credential。
